...
public class NewMember extends JFrame {
	...
	public NewMember() {
		...
		JLabel lblMemberId = new JLabel("Name:-");
		...
		name.setColumns(10);
		name.setBounds(184, 91, 206, 25);
		main.add(name);
		
		JLabel v1 = new JLabel("*");
		v1.setForeground(Color.RED);
		v1.setBounds(400, 95, 141, 23);
		main.add(v1);
		...
		gymTime.setBounds(693, 322, 206, 27);
		main.add(gymTime);
		
		JLabel v2 = new JLabel("*");
		v2.setForeground(Color.RED);
		v2.setBounds(909, 93, 125, 23);
		main.add(v2);
		
		JLabel v4 = new JLabel("*");
		v4.setForeground(Color.RED);
		v4.setBounds(909, 150, 125, 23);
		main.add(v4);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e)
			{
				...
				String agg=age.getText();
				String fees=fee.getText();
				String time=(String) gymTime.getSelectedItem();
				String dat=date.getText();
				boolean maleFlag=male.isSelected();
				boolean femaleFlag=female.isSelected();
				String g=null;
				String timing=(String) gymTime.getSelectedItem();
				...
				else if(sur.trim().isEmpty())
				{
					v2.setText("This field is required.");
				}
				
				else if(con.trim().isEmpty())
				{
					v3.setText("This field is required.");
				}
				
				else  if(conLength<10)
				{
					v3.setText("Invalid number,Pls enter 10 digit No."); 
				}
				
				else  if(conLength>10)
				   {
					v3.setText("Number should not Exceed 10");
				   }
				...
				{
					v7.setText("This field is required.");
				}
				
				else if(maleFlag==false && femaleFlag==false)
				{
					v8.setText("Select Gender");
				}
				
				else if(maleFlag==true && femaleFlag==true)
				{
					v8.setText("Select only one");
				}
				
				else if(timing.trim().isEmpty())
				{
					v9.setText("This field is required.");
				}
				
				else if(dat.trim().isEmpty())
				{
					v10.setText("This field is required.");
				}
				...
				else
				{
					...
					Member m=new Member(nam, sur, add, con, emailId, g, agg, time, fees,dat);
					MemberDaoImp mdao=new MemberDaoImp();
					boolean flag=mdao.addMember(m);
		...
		btnNewButton.setBounds(429, 436, 206, 37);
		...
		lblSurname.setBounds(550, 89, 133, 27);
		main.add(lblSurname);
		
		surname = new JTextField();
		surname.setFont(new Font("Tahoma", Font.ITALIC, 15));
		surname.setColumns(10);
		surname.setBounds(693, 91, 206, 25);
		main.add(surname);
		...
		lblEmail.setBounds(550, 146, 133, 27);
		main.add(lblEmail);
		
		email = new JTextField();
		email.setFont(new Font("Tahoma", Font.ITALIC, 15));
		email.setColumns(10);
		email.setBounds(693, 148, 206, 25);
		main.add(email);
		...
		lblAge.setBounds(550, 204, 133, 27);
		main.add(lblAge);
		
		age = new JTextField();
		age.setFont(new Font("Tahoma", Font.ITALIC, 15));
		age.setColumns(10);
		age.setBounds(693, 206, 206, 25);
		main.add(age);
		...
		lblAdmissionFee.setBounds(533, 264, 161, 27);
		main.add(lblAdmissionFee);
		
		fee = new JTextField();
		fee.setFont(new Font("Tahoma", Font.ITALIC, 15));
		fee.setColumns(10);
		fee.setBounds(693, 266, 206, 25);
		main.add(fee);
		...
		lblFeePaid.setBounds(551, 319, 133, 27);
		main.add(lblFeePaid);
